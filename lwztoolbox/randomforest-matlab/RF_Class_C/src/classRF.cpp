...
void classRF(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
    ...
    int nsample0, mdim, nclass, addClass, mtry, ntest, nsample, ndsize,
            mimp, nimp, mynear, nuse, noutall, nrightall, nrightimpall,
            keepInbag, nstrata;
    ...
    if (oobprox) {
        oobpair = (int *) S_alloc_alt(mynear*mynear, sizeof(int));
    }
    //printf("nsample=%d\n", nsample);
    ...
    for(jb = 0; jb < Ntree; jb++) {
        ...
        /*  DO PROXIMITIES */
        if (iprox) {
            computeProximity(prox, oobprox, nodex, jin, oobpair, mynear);
    ...
    if (iprox) {
        for (n = 0; n < mynear; ++n) {
            for (k = n + 1; k < mynear; ++k) {
                prox[mynear*k + n] /= oobprox ?
                    (oobpair[mynear*k + n] > 0 ? oobpair[mynear*k + n] : 1) :
                        Ntree;
                        prox[mynear*n + k] = prox[mynear*k + n];
            }
            prox[mynear*n + n] = 1.0;
        }
        if (*testdat) {
